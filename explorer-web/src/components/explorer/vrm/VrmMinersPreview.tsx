...
export function VrmMinersPreview({ miners: initialMiners }: { miners: MinersLeaderboardResult }) {
  ...
  return (
    <BcPanel
      title="Top miners"
      action={<ChainPanelLink href={`/vrm/miners?period=${period}`} label="Miners" />}
    >
      <div className="mb-3">
        <MinersPeriodPicker
          period={period}
      ...
      {error ? (
        ...
        <p className="text-sm text-fg-muted">{formatExplorerUserMessage(miners.message)}</p>
