...
function FlowArrow() {
  return (
    <div className="hidden shrink-0 items-center justify-center px-2 text-fg-subtle sm:flex" aria-hidden>
...
function FlowNode({
  ...
  return (
    <div className={cn("min-w-0 flex-1 rounded-lg border px-3 py-2.5", toneClasses[tone])}>
      ...
      {address ? (
        <div className="mt-1">
          <MonoLink href={`/vrm/address/${address}`} value={address} maxLength={18} />
        </div>
      ) : (
        <div className="mt-1 text-xs text-fg-muted">coinbase</div>
      )}
      <div className="mt-1 font-mono text-sm font-semibold tabular-nums text-fg">{amount}</div>
    </div>
  );
}
...
export function TxFlowDiagram({ result }: { result: TransactionResult }) {
  ...
  const flowBody = isMining ? (
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2">
        <div className="text-[11px] font-medium uppercase tracking-wide text-fg-subtle">Outputs</div>
        <div className="flex flex-col gap-2">
      ...
      {totals ? (
        <div className="flex shrink-0 items-start lg:max-w-[120px]">
          <div className="rounded-lg border border-border/70 bg-bg-subtle/60 px-3 py-2.5">
            <div className="text-[10px] font-medium uppercase tracking-wide text-fg-subtle">Fee</div>
...
export function TxFlowMetaLink({ blockHeight }: { blockHeight: number }) {
  return (
    <Link href={`/vrm/block/${blockHeight}`} className="text-accent hover:underline">
...
export function TxFlowHashHint({ txid }: { txid: string }) {
  return <span className="font-mono text-xs text-fg-subtle">{ellipsizeMiddle(txid, 20)}</span>;
}
