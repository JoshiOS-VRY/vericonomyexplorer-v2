    ...
    <section className="rounded-xl border border-border bg-bg-panel px-5 py-4 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        {Array.from({ length: 2 }).map((_, index) => (
          <div
            key={index}
            className="h-16 animate-pulse rounded-md bg-bg-subtle"
          />
        ))}
      </div>
    </section>
    ...
    return [
      {
        ...
        transactions: result.value.data.transactions
...
export function TxRelatedActivityLoader({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const nextGroups = await loadRelatedTransactions(
          chainId,
          txid,
          addresses,
        );
        ...
        {groups.map((group) => (
          <div key={group.address} className="px-5 py-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <Link
                href={chainAddressPath(chainId as ChainId, group.address)}
            ...
            {group.transactions.length === 0 ? (
              ...
              <ul className="space-y-2">
                {group.transactions.map((tx) => (
                  <li key={tx.txid}>
                    <Link
                      href={chainTxPath(chainId as ChainId, tx.txid)}
