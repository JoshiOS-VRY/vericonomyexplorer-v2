...
export default async function BlockPage({
  ...
  try {
    ...
    if (block.success === false) {
      return <AlertBanner title="Block Not Found">No block matched this hash or height.</AlertBanner>;
    }
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <SummaryRow
              items={[
                { label: "Hash", value: <MonoLink href={`/block/${String(block.hash)}`} value={String(block.hash)} maxLength={32} /> },
          ...
          <CardContent>
            <div className="space-y-2">
              {txids.map((txid, index) => (
                <div key={txid} className="flex items-center gap-3 border-b border-border py-2 last:border-0">
