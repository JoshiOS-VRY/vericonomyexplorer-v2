...
export default async function TxPage({ params }: { params: Promise<{ txid: string }> }) {
  ...
  try {
    ...
    if (data.success === false) {
      return (
        <AlertBanner title="Transaction Not Found">No transaction matched this txid.</AlertBanner>
      );
    }
    ...
    return (
      <div className="space-y-6">
          ...
          <CardContent>
            <SummaryGrid
              items={[
                {
                  label: 'Txid',
                  value: <MonoLink href={`/tx/${txid}`} value={txid} maxLength={36} />,
                },
                {
                  label: 'Block',
                  value: data.blockhash ? (
                    <Link
                      href={`/block/${String(data.blockhash)}`}
                ...
                {
                  label: 'Time',
                  value: data.time ? <TimeCell time={Number(data.time)} /> : 'Unconfirmed',
                },
              ]}
            />
          </CardContent>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <IoTable
            title="Inputs"
            rows={vin.map((input, index) => [
              String(index),
              input.coinbase ? (
                'coinbase'
              ) : input.txid ? (
                <MonoLink
                  href={`/tx/${String(input.txid)}`}
                  value={String(input.txid)}
                  maxLength={20}
                />
              ) : (
                '—'
          ...
          <IoTable
            title="Outputs"
            rows={vout.map((output, index) => [
              String(index),
              output.scriptPubKey && typeof output.scriptPubKey === 'object'
                ? String(
                    (output.scriptPubKey as { addresses?: string[] }).addresses?.[0] ?? 'script'
                  )
                : '—',
