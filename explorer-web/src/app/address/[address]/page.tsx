...
export default async function AddressPage({
  ...
  try {
    ...
    if (data.success === false) {
      return <AlertBanner title="Address Not Found">No data available for this address.</AlertBanner>;
    }
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="font-mono text-sm break-all text-fg-muted">{address}</p>
        <Card>
          ...
          <CardContent>
            <SummaryGrid
              items={[
                ...
                { label: "Total Received", value: data.totalReceived != null ? String(data.totalReceived) : "N/A" },
                { label: "Total Sent", value: data.totalSent != null ? String(data.totalSent) : "N/A" },
              ]}
            />
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {txids.length === 0 ? (
              ...
              <DataTable
                headers={["#", "Txid"]}
                rows={txids.slice(0, 50).map((txid, index) => [
                  String(index + 1),
            ...
            {txids.length > 50 ? (
              <p className="mt-3 text-sm text-fg-muted">Showing first 50 of {txids.length} transactions.</p>
            ) : null}
          </CardContent>
        </Card>
      </div>
