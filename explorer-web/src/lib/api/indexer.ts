...
export async function getIndexerHealth(): Promise<IndexerHealth> {
  const data = await v1Fetch<IndexerHealth>('/indexer/status', {
...
export async function getChainActivityHistory(
  ...
  const qs = search.toString();
  return v1Fetch<ChainActivityHistoryResult>(`/${chainId}/activity-history${qs ? `?${qs}` : ''}`, {
    revalidate: SUMMARY_REVALIDATE_SECONDS,
  });
}

export async function getChainSummary(chainId: string): Promise<ChainSummary> {
  const data = await v1Fetch<unknown>(`/${chainId}/summary`, {
    revalidate: SUMMARY_REVALIDATE_SECONDS,
  });
...
export async function getTransaction(chainId: string, txid: string): Promise<TransactionResult> {
