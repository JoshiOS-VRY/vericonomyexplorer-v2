...
describe('liveBlocksMerge', () => {
  ...
  it('rejects tip-stream stub rows for full table readiness', () => {
    ...
    expect(isIndexedBlockTableReady({ ...block(101), interestRatePercent: null }, 'vrc')).toBe(
      false
    );
  });
  ...
  it('keeps an optimistic tip stub at the top for home display', () => {
    ...
    const displayed = mergeBlocksForDisplay([stub, block(100), block(99)], 'vrm');
  ...
  it('drops non-tip optimistic rows from display', () => {
    ...
    const displayed = mergeBlocksForDisplay([block(100), staleStub, block(99)], 'vrm');
