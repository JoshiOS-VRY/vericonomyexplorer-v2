    ...
    expect(
      chainSummarySchema.parse({
        ...
        source: {
          ...
          message:
            'Historical balances are internally consistent, but the index is still catching up.',
        },
      })
    ).toBeTruthy();
  });
  ...
  it('accepts disabled richlist payload', () => {
    expect(
      richlistSchema.parse({
        ...
        trusted: false,
        message:
          'richlist is disabled until this chain is indexed from genesis, internally consistent, and near tip.',
        source: {
  ...
  it('accepts disabled leaderboard payload', () => {
    expect(
      leaderboardSchema.parse({
        ...
        trusted: false,
        message:
          'leaderboards is disabled until this chain is indexed from genesis, internally consistent, and near tip.',
        source: {
          label: 'disabled-until-trusted',
          trustLevel: 'historical',
          healthStatus: 'syncing',
