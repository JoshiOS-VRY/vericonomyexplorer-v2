...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	if (match) {
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Verium Core?`);
	}
...
async function assessTxindexAvailability() {
	...
	try {
		...
		let isMethodNotFound = errorString.includes("-32601") || errorString.includes("Method not found");
		...
		if (isMethodNotFound) {
			...
			debugLog("txindex check: getindexinfo method not found, treating as unsupported, trying txid lookup");
			...
			try {
				...
				let isMethodNotFound2 = errorString2.includes("-32601") || errorString2.includes("Method not found");
