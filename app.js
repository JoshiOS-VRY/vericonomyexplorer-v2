...
if (config.cdn.active) {
  cdnS3Bucket = s3Utils.createBucket(
    config.cdn.s3Bucket,
    config.cdn.s3BucketRegion,
    config.cdn.s3BucketPath
  );
}
...
const indexerPageRouter = require('./routes/indexerPageRouter.js');
...
const statsProcessFunction = (name, stats) => {
  ...
  if (process.env.STATS_API_URL) {
    ...
    axios
      .post(process.env.STATS_API_URL, data)
      .then((res) => {
...
const processStatsInterval = setInterval(
  () => {
    statTracker.processAndReset(statsProcessFunction, statsProcessFunction, statsProcessFunction);
  },
...
expressApp.use(
  require('./app/actionPerformanceMonitor.js')(statTracker, {
...
if (process.env.NODE_ENV != 'local') {
  ...
  debugLog(
    'Enabling view caching (performance will be improved but template edits will not be reflected)'
  );
...
if (rateLimitWindowMinutes == -1) {
  ...
  debugLog(
    `Enabling rate limiting: ${rateLimitWindowMaxRequests} requests per ${rateLimitWindowMinutes}min`
  );

  const rateLimiter = rateLimit({
    windowMs: rateLimitWindowMinutes * 60 * 1000, // 15 minutes
    limit: function (req, res) {
      // Apply stricter limits for crawlers
      const userAgent = req.get('user-agent') || '';
      const crawler = utils.getCrawlerFromUserAgentString(userAgent);

      if (crawler) {
        return config.rateLimiting.crawlerWindowMaxRequests || 100;
      }

      return rateLimitWindowMaxRequests;
    },
    ...
    handler: function (req, res, next) {
      ...
      if (crawler) {
        debugLog(`Rate-limiting crawler ${crawler}: ${req.url}`);
      } else {
        debugErrorLog(
          `Rate-limiting request: req=${JSON.stringify(utils.expressRequestToJson(req))}`
        );
      }
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
  ...
  var services = getnetworkinfo.localservicesnames
    ? '[' + getnetworkinfo.localservicesnames.join(', ') + ']'
    : getnetworkinfo.localservices;
  ...
  if (match) {
    ...
    debugErrorLog(
      `Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Verium Core?`
    );
  }

  debugLog(
    `RPC Connected: version=${getnetworkinfo.version} subversion=${getnetworkinfo.subversion}, parsedVersion(used for RPC versioning)=${global.btcNodeSemver}, protocolversion=${getnetworkinfo.protocolversion}, chain=${getblockchaininfo.chain}, services=${services}`
  );
  ...
  if (config.sqliteEnabled) {
    ...
    // Start sync service with backfill enabled
    // Backfill will start 30 seconds after startup to sync historical blocks
    sqliteSync.startSyncService(5, true); // Sync every 5 minutes, start backfill immediately
    debugLog('SQLite sync service started (with backfill)');
  }
...
async function monitorNewTransactions() {
  ...
  for await (const [topic, message] of sock) {
    ...
    console.log(topic.toString('ascii') + ' - ' + message.toString('hex'));
  }
}
...
async function loadDifficultyHistory(tipBlockHeight = null) {
  ...
  if (config.slowDeviceMode) {
    debugLog(
      "Skipping performance-intensive task: load difficulty history. This is skipped due to the flag 'slowDeviceMode' which defaults to 'true' to protect slow nodes. Set this flag to 'false' to enjoy difficulty history details."
    );
  ...
  try {
    ...
    if (global.getindexinfo.txindex) {
      ...
      try {
        ...
        let knownTx = await coreApi.getRawTransaction(
          coinConfig.knownTransactionsByNetwork[global.activeBlockchain]
        );
    ...
    // Check if the error is "Method not found" (code -32601) - this means the RPC method doesn't exist
    // This can happen with forks like Verium that don't support all Bitcoin Core RPC methods
    let errorString = JSON.stringify(e);
    let isMethodNotFound =
      errorString.includes('-32601') || errorString.includes('Method not found');

    if (isMethodNotFound) {
      // Method doesn't exist, treat as unsupported and fall back to txid lookup
      // Don't log this error as it's expected for unsupported methods
      debugLog(
        'txindex check: getindexinfo method not found, treating as unsupported, trying txid lookup'
      );

      try {
        // Test if getrawtransaction works without a blockhash - this requires txindex
        // We'll use rpcApi directly to bypass the fallback logic in coreApi
        let knownTxId = coinConfig.knownTransactionsByNetwork[global.activeBlockchain];
        debugLog(`txindex check: testing txid lookup for: ${knownTxId}`);

        // Try to get the transaction by txid only (no blockhash) - this requires txindex
        // Using rpcApi directly bypasses the noTxIndexTransactionLookup fallback
        let testResult = await rpcApi.getRpcDataWithParams({
          method: 'getrawtransaction',
          parameters: [knownTxId, 1], // verbose mode, no blockhash
        });

        // If we get here, the RPC call succeeded, which means txindex is available
        global.txindexAvailable = true;
        debugLog('txindex check: available! (getindexinfo not supported, but txid lookup works)');
      } catch (e2) {
        // Check if this is also a "Method not found" or if it's a real txindex issue
        let errorString2 = JSON.stringify(e2);
        let isMethodNotFound2 =
          errorString2.includes('-32601') || errorString2.includes('Method not found');

        if (isMethodNotFound2) {
          // getrawtransaction itself might not be available (unlikely but possible)
          debugLog('txindex check: getrawtransaction method also not found');
          global.txindexAvailable = false;
        } else {
          // The transaction lookup failed - this could mean:
          // 1. txindex is not enabled
          // 2. The known transaction doesn't exist
          // 3. Some other error
          debugLog(`txindex check: txid lookup failed: ${errorString2}`);
          debugLog('txindex check: assuming txindex is unavailable');
          global.txindexAvailable = false;
        }
      }
    } else {
      // Other error, log it and retry
      utils.logError('o2328ryw8wsde', e);

      var retryTime = parseInt(
        Math.min(15 * 60 * 1000, 1000 * 10 * Math.pow(2, txindexCheckCount))
      );
      txindexCheckCount++;
...
async function refreshUtxoSetSummary() {
  if (config.slowDeviceMode) {
    if (!global.getindexinfo || !global.getindexinfo.coinstatsindex) {
      ...
      debugLog(
        "Skipping performance-intensive task: fetch UTXO set summary. This is skipped due to the flag 'slowDeviceMode' which defaults to 'true' to protect slow nodes. Set this flag to 'false' to enjoy UTXO set summary details."
      );
...
function refreshNetworkVolumes() {
  if (config.slowDeviceMode) {
    debugLog(
      "Skipping performance-intensive task: fetch last 24 hrs of blockstats to calculate transaction volume. This is skipped due to the flag 'slowDeviceMode' which defaults to 'true' to protect slow nodes. Set this flag to 'false' to enjoy UTXO set summary details."
    );
  ...
  coreApi.getBlockchainInfo().then(function (result) {
    ...
    Promise.all(promises).then(function (results) {
      ...
      if (results && results.length > 0 && results[0] != null) {
        ...
        debugLog(
          'Unable to load network volume, likely due to bitcoind version older than 0.17.0 (the first version to support getblockstats).'
        );
      }
    });
  });
...
expressApp.onStartup = async () => {
  ...
  debugLog(
    `Environment(${expressApp.get('env')}) - Node: ${process.version}, Platform: ${process.platform}, Versions: ${JSON.stringify(process.versions)}`
  );
  ...
  if (global.sourcecodeVersion == null && fs.existsSync('.git')) {
    try {
      ...
      debugLog(
        `Starting ${global.coinConfig.ticker} RPC Explorer, v${global.appVersion} (commit: '${global.sourcecodeVersion}', date: ${global.sourcecodeDate}) at http://${config.host}:${config.port}${config.baseUrl}`
      );
    } catch (err) {
      ...
      global.cacheId = global.appVersion;
      debugLog(
        `Error getting sourcecode version, continuing to use default cacheId '${global.cacheId}'`
      );

      debugLog(
        `Starting ${global.coinConfig.ticker} RPC Explorer, v${global.appVersion} (code: unknown commit) at http://${config.host}:${config.port}${config.baseUrl}`
      );
    }
    ...
    global.cacheId = global.appVersion;
    debugLog(
      `No sourcecode version available, continuing to use default cacheId '${global.cacheId}'`
    );

    debugLog(
      `Starting ${global.coinConfig.ticker} RPC Explorer, v${global.appVersion} at http://${config.host}:${config.port}${config.baseUrl}`
    );
  ...
  if (config.cdn.active && config.cdn.s3Bucket) {
    ...
    const s3Path = (filepath) => {
      return `${global.cacheId}/${filepath}`;
    };
...
function connectToRpcServer() {
  ...
  debugLog(
    `RPC Credentials: ${JSON.stringify(utils.obfuscateProperties(credentialsForRpcConnect, ['password']), null, 4)}`
  );
  ...
  debugLog(
    `RPC Connection properties: ${JSON.stringify(utils.obfuscateProperties(rpcClientProperties, ['password']), null, 4)}`
  );
...
expressApp.continueStartup = function () {
  ...
  if (config.addressApi) {
    ...
    if (!supportedAddressApis.includes(config.addressApi)) {
      utils.logError(
        '32907ghsd0ge',
        `Unrecognized value for BTCEXP_ADDRESS_API: '${config.addressApi}'. Valid options are: ${supportedAddressApis}`
      );
    }
    ...
    if (config.addressApi == 'electrum' || config.addressApi == 'electrumx') {
      if (config.electrumServers && config.electrumServers.length > 0) {
        ...
        utils.logError(
          '327hs0gde',
          "You must set the 'BTCEXP_ELECTRUM_SERVERS' environment variable when BTCEXP_ADDRESS_API=electrum."
        );
      }
    }
  }
...
expressApp.use(function (req, res, next) {
  ...
  userSettings.displayCurrency =
    userSettings.displayCurrency || config.displayDefaults.displayCurrency;
  ...
  const indexerOnlyRequest = isIndexerOnlyRequest(req);

  if (!indexerOnlyRequest && !['/', '/connect'].includes(req.originalUrl)) {
  ...
  if (!indexerOnlyRequest && !global.rpcConnected) {
    res.status(500);
    res.render('error', {
...
function isIndexerOnlyRequest(req) {
...
expressApp.use(config.baseUrl, indexerPageRouter);
...
expressApp.use(function (req, res, next) {
  ...
  if (crawler) {
    debugAccessLog(
      `Finished action '${req.path}' (${res.statusCode}) in ${time}ms for crawler '${crawler}' / '${userAgent}', ip=${ip}`
    );
  } else {
    debugAccessLog(
      `Finished action '${req.path}' (${res.statusCode}) in ${time}ms for UA '${userAgent}', ip=${ip}`
    );
  }
...
expressApp.use(function (req, res, next) {
  // Skip tracking/logging for obvious bot spam patterns
  const path = req ? req.url : 'unknown url';
  const userAgent = req ? req.get('user-agent') || '' : '';
  const crawler = utils.getCrawlerFromUserAgentString(userAgent);

  // Common bot spam patterns (malformed URLs, repeated patterns, etc.)
  const spamPatterns = [
    /\/block\/block-analysis\//, // Malformed block-analysis URLs
    /\/block\/block\//, // Double "block" in path
    /\.(php|asp|jsp|html)$/i, // Common exploit attempts
    /\.\./, // Path traversal attempts
    /\/wp-admin|\/wp-content|\/wp-includes/i, // WordPress exploit attempts
    /\/admin|\/administrator/i, // Admin panel attempts
  ];

  const isSpamPattern = spamPatterns.some((pattern) => pattern.test(path));

  // Only track/log if not a crawler or spam pattern
  if (!crawler && !isSpamPattern) {
    utils.trackAppEvent('error404');
  }

  var err = new Error(`Not Found: ${path}`);
  err.status = 404;
  err.isSpamPattern = isSpamPattern;
  err.crawler = crawler;
...
const sharedErrorHandler = (req, err) => {
  if (err && err.message && err.message.includes('Not Found')) {
    ...
    // Reduce log spam: only log 404s for non-crawlers and non-spam patterns
    // Skip logging for obvious bot spam (malformed URLs, exploit attempts, etc.)
    const shouldLog = !err.isSpamPattern && !crawler;

    if (shouldLog) {
      debugErrorLog(`404 NotFound: path=${path}, ip=${ip}, userAgent=${userAgent}`);
      utils.logError(`NotFound`, err, attributes, false);
    } else if (crawler) {
      // Only debug log for crawlers (less verbose)
      debugLog(`404 (crawler ${crawler}): ${path}`);
    }
  } else {
