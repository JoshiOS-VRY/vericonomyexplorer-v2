...
export async function registerChainRoutes(app) {
    ...
    app.get('/v1/:chain/summary/lite', { ...liveReadRateLimitRouteConfig }, async (request, reply) => {
        const chainId = parseChainId(request.params.chain);
        if (!chainId) {
            return reply.code(400).send({ error: 'Invalid chain id' });
        }
    ...
    app.get('/v1/:chain/blocks/latest', { ...liveReadRateLimitRouteConfig }, async (request, reply) => {
        const chainId = parseChainId(request.params.chain);
        if (!chainId) {
            return reply.code(400).send({ error: 'Invalid chain id' });
        }
