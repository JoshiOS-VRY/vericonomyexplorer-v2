...
export async function registerTipRoutes(app) {
    app.get('/v1/:chain/tip', { ...liveReadRateLimitRouteConfig }, async (request, reply) => {
    ...
    app.get('/v1/:chain/tip/height', { ...liveReadRateLimitRouteConfig }, async (request, reply) => {
        const chainId = parseChainId(request.params.chain);
        if (!chainId) {
            return reply.code(400).send({ error: 'Invalid chain id' });
        }
    ...
    app.get('/v1/:chain/tip/stream', { ...liveReadRateLimitRouteConfig }, async (request, reply) => {
        const chainId = parseChainId(request.params.chain);
        if (!chainId) {
            return reply.code(400).send({ error: 'Invalid chain id' });
        }
