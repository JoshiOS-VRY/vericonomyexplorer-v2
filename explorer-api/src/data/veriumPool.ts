...
function isVeriumAddress(value: string): boolean {
  return VERIUM_ADDRESS_RE.test(value.trim());
}

/** Legacy wallet `miner.address` must be a chain address, not a display label. */
export function resolveWalletMinerAddress(
  extractedBy: unknown,
  extractedByAddress: unknown,
): string | null {
  const address =
    typeof extractedByAddress === "string" ? extractedByAddress.trim() : "";
  if (address) return address;

  const label = typeof extractedBy === "string" ? extractedBy.trim() : "";
  if (!label) return null;
  if (label === VERIUM_POOL_DISPLAY_NAME) return VERIUM_POOL_PAYOUT_ADDRESS;
  if (isVeriumAddress(label)) return label;
  return null;
}
