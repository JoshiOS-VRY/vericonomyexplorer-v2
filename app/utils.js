...
function formatCurrencyAmountWithForcedDecimalPlaces(amount, formatType, forcedDecimalPlaces) {
  ...
  if (currencyType.type == 'native') {
    ...
    if (forcedDecimalPlaces >= 0) {
      ...
      return {
        val: baseStr,
        currencyUnit: currencyType.name,
        simpleVal: baseStr,
        intVal: parseInt(dec),
      };
    } else {
      ...
      // of the UX benefit of larger, "even" amounts (e.g. 0.1VRM).
      ...
      if (baseStr.indexOf('.') == -1) {
        ...
        if (baseStr.length - baseStr.indexOf('.') - 1 > maxValDigits) {
          ...
          returnVal.lessSignificantDigits = baseStr.substring(
            baseStr.indexOf('.') + maxValDigits + 1
          );
        } else {
          returnVal.val = baseStr;
        }
    ...
    if (global.exchangeRates != null && global.exchangeRates[currencyType.id] != null) {
      ...
      return formatCurrencyAmountWithForcedDecimalPlaces(
        amount,
...
function formatCurrencyAmountInSmallestUnits(amount, forcedDecimalPlaces) {
  return formatCurrencyAmountWithForcedDecimalPlaces(
    amount,
    coins[config.coin].baseCurrencyUnit.name,
    forcedDecimalPlaces
  );
}
...
function satoshisPerUnitOfLocalCurrency(localCurrency) {
  if (global.exchangeRates != null) {
    ...
    return {
      amt: addThousandsSeparators(exchangedAmt),
      amtRaw: exchangedAmt,
      unit: `sat/${localCurrencyType.symbol}`,
    };
  }
...
function ellipsizeMiddle(str, length, replacement = '…', extraCharAtStart = true) {
  if (str.length <= length) {
    ...
    if ((length - replacement.length) % 2 == 0) {
      return (
        str.substring(0, (length - replacement.length) / 2) +
        replacement +
        str.slice(-(length - replacement.length) / 2)
      );
    } else {
      if (extraCharAtStart) {
        return (
          str.substring(0, Math.ceil((length - replacement.length) / 2)) +
          replacement +
          str.slice(-Math.floor((length - replacement.length) / 2))
        );
      } else {
        return (
          str.substring(0, Math.floor((length - replacement.length) / 2)) +
          replacement +
          str.slice(-Math.ceil((length - replacement.length) / 2))
        );
      }
    }
  }
...
function summarizeDuration(duration, options = {}) {
  ...
  let shortenDurationNames =
    'shortenDurationNames' in options ? options.shortenDurationNames : true;
    ...
    if (txInputs) {
      for (let i = 0; i < tx.vin.length; i++) {
        if (tx.vin[i].coinbase) {
          totalInputValue = totalInputValue.plus(
            new Decimal(coinConfig.blockRewardFunction(blockHeight, global.activeBlockchain))
          );
        } else {
...
function estimatedSupply(height) {
  ...
  while (i < height) {
    ...
    if (height < nextHalvingHeight) {
      ...
      return supply.plus(
        new Decimal(heightDiff).times(coinConfig.blockRewardFunction(i, global.activeBlockchain))
      );
    }
...
async function refreshExchangeRates() {
  ...
  if (coins[config.coin].exchangeRateData) {
    try {
      ...
      let exchangeRates = coins[config.coin].exchangeRateData.responseBodySelectorFunction(
        response.data
      );
      if (exchangeRates != null) {
        ...
        debugLog(
          'Using exchange rates: ' +
            JSON.stringify(global.exchangeRates) +
            ' starting at ' +
            global.exchangeRatesUpdateTime
        );
      } else {
  ...
  if (coins[config.coin].goldExchangeRateData) {
    if (process.env.NODE_ENV == 'local') {
      ...
      debugLog(
        'Using DEBUG gold exchange rates: ' +
          JSON.stringify(global.goldExchangeRates) +
          ' starting at ' +
          global.goldExchangeRatesUpdateTime
      );
    } else {
      try {
        const response = await axios.get(coins[config.coin].goldExchangeRateData.jsonUrl);

        let exchangeRates = coins[config.coin].goldExchangeRateData.responseBodySelectorFunction(
          response.data
        );
        if (exchangeRates != null) {
          ...
          ipCache.get(ipStr).then(async function (result) {
            if (result.value == null) {
              let apiUrl =
                ...
                result.key +
                '?access_key=' +
                config.credentials.ipStackComApiAccessKey;
              ...
              try {
                ...
                if (response.data.latitude && response.data.longitude) {
                  debugLog(
                    `Successful IP-geo-lookup: ${ip} -> (${response.data.latitude}, ${response.data.longitude})`
                  );
                } else {
...
function formatLargeNumberSignificant(n, significantDigits) {
  try {
    for (let i = 0; i < exponentScales.length; i++) {
      ...
      if (Math.abs(fraction) >= 1) {
        return [
          fraction.toDP(Math.max(0, significantDigits - `${Math.floor(fraction)}`.length)),
          item,
        ];
      }
    }
  ...
  debugErrorLog(
    'Error ' +
      ...
      (optionalUserData != null
        ? ', userData: ' + optionalUserData + ' (json: ' + JSON.stringify(optionalUserData) + ')'
        : '')
  );
...
const bip32TestnetNetwork = {
  messagePrefix: '\x18Verium Signed Message:\n',
  bech32: 'tb',
  bip32: {
    public: 0x043587cf,
    private: 0x04358394,
    ...
    if (addressType == 'p2pkh') {
      ...
      addresses.push(
        bitcoinjs.payments.p2sh({
          redeem: bitcoinjs.payments.p2wpkh({ pubkey: publicKey, network: network }),
        }).address
      );
      ...
      throw new Error(
        `Unknown address type: "${addressType}" (should be one of ["p2pkh", "p2sh(p2wpkh)", "p2wpkh"])`
      );
    }
  }
...
function difficultyAdjustmentEstimates(eraStartBlockHeader, currentBlockHeader) {
  let difficultyPeriod = parseInt(
    Math.floor(currentBlockHeader.height / coinConfig.difficultyAdjustmentBlockCount)
  );
  let blocksUntilDifficultyAdjustment =
    (difficultyPeriod + 1) * coinConfig.difficultyAdjustmentBlockCount - currentBlockHeader.height;
  ...
  let daysUntilAdjustment = new Decimal(blocksUntilDifficultyAdjustment)
    .times(timePerBlock)
    .dividedBy(60 * 60 * 24);
  let hoursUntilAdjustment = new Decimal(blocksUntilDifficultyAdjustment)
    .times(timePerBlock)
    .dividedBy(60 * 60);
  let duaDP1 = daysUntilAdjustment.toDP(1);
  let daysUntilAdjustmentStr =
    daysUntilAdjustment > 1 ? `~${duaDP1} day${duaDP1 == '1' ? '' : 's'}` : '< 1 day';
  let hoursUntilAdjustmentStr =
    hoursUntilAdjustment > 1
      ? `~${hoursUntilAdjustment.toDP(0)} hr${hoursUntilAdjustment.toDP(1) == '1' ? '' : 's'}`
      : '< 1 hr';
...
function nextHalvingEstimates(
  eraStartBlockHeader,
  currentBlockHeader,
  difficultyAdjustmentDataArg = null
) {
  ...
  if (!difficultyAdjustmentData) {
    difficultyAdjustmentData = difficultyAdjustmentEstimates(
      eraStartBlockHeader,
      currentBlockHeader
    );
  }

  let blockCountAffectedByCurrentDifficultyDelta = Math.min(
    difficultyAdjustmentData.blocksLeft,
    blocksUntilNextHalving
  );
  let currDifficultyEraTimeDifferential =
    (coinConfig.targetBlockTimeSeconds - difficultyAdjustmentData.timePerBlock) *
    blockCountAffectedByCurrentDifficultyDelta;

  let secondsUntilNextHalving =
    blocksUntilNextHalving * targetBlockTimeSeconds - currDifficultyEraTimeDifferential;
...
function tryParseAddress(address) {
  ...
  // Normalize address to lowercase for bech32/bech32m parsing (bech32 is case-insensitive but library requires consistent case)
  let addressLower = address.toLowerCase();

  // Try bech32/bech32m if:
  // 1. Address contains a separator character "1" (standard bech32), OR
  // 2. Address starts with "v" (could be Verium bech32 with HRP "vrm" - even without separator, let RPC validate)
  let hasBech32Separator = addressLower.includes('1');
  let couldBeVeriumBech32 = addressLower.startsWith('v');
  let shouldTryBech32 = hasBech32Separator || couldBeVeriumBech32;

  if (shouldTryBech32) {
    try {
      parsedAddress = bitcoinjs.address.fromBech32(addressLower);
    ...
    try {
      parsedAddress = bech32m.decode(addressLower);
  ...
  // Only include errors for addresses that looked like they should be valid
  // Filter out common "format doesn't match" errors to avoid noise
  // For Verium addresses starting with "v", we'll let the RPC validateaddress be the final authority
  if (base58Error && address.match(b58prefix)) {
    // Only include base58 error if address matched the base58 prefix pattern
    returnVal.errors.push(base58Error);
  }

  // For bech32 errors:
  // - If address has separator: include error (unless it's just "No separator character")
  // - If address starts with "v" but no separator: don't include error (let RPC validateaddress decide)
  //   This handles Verium addresses that might be valid but don't follow standard bech32 format
  if (bech32Error && shouldTryBech32) {
    let errorMsg = bech32Error.message || JSON.stringify(bech32Error);
    // Filter out "No separator character" errors - these are format mismatches, not real errors
    // For Verium addresses starting with "v", we'll let RPC validateaddress be the authority
    if (!errorMsg.includes('No separator character')) {
      // Only include if it's a real parsing error, not just a format mismatch
      returnVal.errors.push(bech32Error);
    }
  }

  if (bech32mError && shouldTryBech32) {
    let errorMsg = bech32mError.message || JSON.stringify(bech32mError);
    // Filter out "No separator character" errors
    if (!errorMsg.includes('No separator character')) {
