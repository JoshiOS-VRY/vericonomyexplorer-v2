const buildNormalizingRegexes = (baseUrl) => {
  return [
    ...
    {
      regex: new RegExp(`^${baseUrl}api/blocks-by-height/.*`, 'i'),
      action: 'api.blocks-by-height',
    },
    {
      regex: new RegExp(`^${baseUrl}api/block-headers-by-height/.*`, 'i'),
      action: 'api.block-headers-by-height',
    },
    {
      regex: new RegExp(`^${baseUrl}api/block-stats-by-height/.*`, 'i'),
      action: 'api.block-stats-by-height',
    },
    { regex: new RegExp(`^${baseUrl}api/mempool-txs/.*`, 'i'), action: 'api.mempool-txs' },
    {
      regex: new RegExp(`^${baseUrl}api/raw-tx-with-inputs/.*`, 'i'),
      action: 'api.raw-tx-with-inputs',
    },
    {
      regex: new RegExp(`^${baseUrl}api/block-tx-summaries/.*`, 'i'),
      action: 'api.block-tx-summaries',
    },
