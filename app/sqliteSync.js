  ...
  try {
    ...
    if (actualMaxHeight && actualMaxHeight.max_height !== null) {
      ...
      if (!metadataHeight || actualHeight > metadataHeight + 1000) {
        debugLog(
          `Sync metadata mismatch detected. Metadata: ${metadataHeight}, Actual: ${actualHeight}. Recovering...`
        );
...
function setLastSyncedHeight(height) {
  ...
  try {
    ...
    ).run('last_synced_height', height.toString(), now);
...
async function syncBlocks(startHeight, endHeight) {
  ...
  try {
    ...
    for (let height = startHeight; height <= endHeight; height++) {
      ...
      if (height % 100 === 0) {
        debugLog(
          `Sync progress: ${height - startHeight + 1}/${endHeight - startHeight + 1} blocks`
        );
      }
    }
...
async function findGapsInBlocks() {
  ...
  try {
    ...
    const range = db
      .prepare(
        'SELECT MIN(height) as min_height, MAX(height) as max_height, COUNT(*) as count FROM blocks;'
      )
      .get();
  ...
  try {
    ...
    if (dbRange && dbRange.max_height !== null) {
      ...
      if (!lastSynced || actualMaxHeight > lastSynced + 1000) {
        debugLog(
          `Metadata recovery: Updating last_synced_height from ${lastSynced} to ${actualMaxHeight}`
        );
...
function startSyncService(intervalMinutes = 5, startBackfillImmediately = false) {
  ...
  if (startBackfillImmediately) {
    ...
    setTimeout(() => {
      startBackfill(null).catch((err) => {
        // null = auto-detect what needs syncing
