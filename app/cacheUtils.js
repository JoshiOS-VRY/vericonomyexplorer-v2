...
function createMemoryLruCache(cacheName, cacheObj, onCacheEvent) {
  return {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (val != null) {
          ...
          if (key.match(watchKeysRegex)) {
            debugLog(
              `cache.${cacheName}[${key}]: HIT  (${utils.addThousandsSeparators(JSON.stringify(val).length)} B)`
            );
          }
        } else {
    ...
    set: (key, obj, maxAge) => {
      ...
      if (key.match(watchKeysRegex)) {
        debugLog(
          `cache.${cacheName}[${key}]: SET  (${utils.addThousandsSeparators(JSON.stringify(obj).length)} B), T=${maxAge}`
        );
      }
