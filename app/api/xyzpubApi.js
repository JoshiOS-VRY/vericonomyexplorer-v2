    ...
    for (let receiveOrChange = 0; receiveOrChange <= 1; receiveOrChange++) {
      if (gapCounts[receiveOrChange] < gapLimit) {
        ...
        if (getAddressResult) {
          ...
          while (moreTx) {
            let detailsResult = await addressApi.getAddressDetails(
              getAddressResult.address,
              getAddressResult.scriptPubKey,
              sort,
              txLimit,
              txOffset
            );
            ...
            if (
              detailsResult &&
              detailsResult.addressDetails &&
              detailsResult.addressDetails.txids
            ) {
