...
const noConnectionsErrorText =
  "No Electrum connection available. This could mean that the connection was lost or that the Electrum server is processing transactions and therefore not accepting requests. This tool will try to reconnect. If you manage your own Electrum server you may want to check your server's logs.";
...
function connectToServer(host, port, protocol) {
  return new Promise(function (resolve, reject) {
    ...
    var electrumClient = new ElectrumClient(
      port,
      host,
...
function runOnServer(electrumClient, f) {
  return new Promise(function (resolve, reject) {
    f(electrumClient)
      ...
      .catch(function (err) {
        utils.logError('ElectrumServerError', err, {
          host: electrumClient.host,
          port: electrumClient.port,
        });
  ...
  return new Promise(function (resolve, reject) {
    ...
    promises.push(
      new Promise(function (resolve2, reject2) {
        getAddressTxids(addrScripthash)
          .then(function (result) {
    ...
    promises.push(
      new Promise(function (resolve2, reject2) {
        getAddressBalance(addrScripthash)
          .then(function (result) {
      ...
      .then(function (results) {
        ...
        if (addrScripthash == coinConfig.genesisCoinbaseOutputAddressScripthash) {
          for (let i = 0; i < results.length; i++) {
            results[i].result.unshift({
              tx_hash: coinConfig.genesisCoinbaseTransactionIdsByNetwork[global.activeBlockchain],
              height: 0,
            });
          }
        }
      ...
      .then(function (results) {
        ...
        if (addrScripthash == coinConfig.genesisCoinbaseOutputAddressScripthash) {
          for (let i = 0; i < results.length; i++) {
            ...
            results[i].result.confirmed +=
              coinbaseBlockReward * coinConfig.baseCurrencyUnit.multiplier;
          }
        }
...
function lookupTxBlockHash(txid) {
  if (electrumClients.length == 0) {
    return Promise.reject({
      ...
      userText: noConnectionsErrorText,
    });
  }
