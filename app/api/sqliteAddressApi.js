...
function getAddressDetails(address, scriptPubkey, sort, limit, offset) {
	return new Promise(function(resolve, reject) {
		...
		try {
			// Get address balance
			const balanceResult = db.prepare("SELECT balance, tx_count FROM address_balances WHERE address = ?;").get(address);
...
function addressExists(address) {
	...
	try {
		...
		const result = db.prepare("SELECT COUNT(*) as count FROM address_txs WHERE address = ? LIMIT 1;").get(address);
