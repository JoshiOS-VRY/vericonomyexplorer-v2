...
const sqliteCache = require('../sqliteCache.js');
...
global.lruCaches = [
  global.miscLruCache,
  global.blockLruCache,
  global.txLruCache,
  global.miningSummaryLruCache,
];
...
(function () {
  const pruneCaches = () => {
    ...
    debugLog(
      `Pruned caches: ${totalSizeBefore.toLocaleString()} -> ${totalSizeAfter.toLocaleString()}`
    );
  };
...
if (!config.noInmemoryRpcCache) {
  ...
  blockCaches.push(
    cacheUtils.createMemoryLruCache('block', global.blockLruCache, onMemoryCacheEvent)
  );
...
if (redisCache.active) {
  ...
  const redisCacheObj = redisCache.createCache(
    `${cacheKeyVersion}-${rpcCredKeyComponent}`,
    onRedisCacheEvent
  );
...
if (sqliteCache.active) {
  global.cacheStats.sqlite = {
    try: 0,
    hit: 0,
    miss: 0,
    error: 0,
  ...
  const onSqliteCacheEvent = function (cacheType, eventType, cacheKey) {
    global.cacheStats.sqlite[eventType]++;
    statTracker.trackEvent(`caches.sqlite.${eventType}`);
    //debugLog(`cache.${cacheType}.${eventType}: ${cacheKey}`);
  };

  // Use the same key prefix pattern as Redis for consistency
  // md5 of the active RPC credentials serves as part of the key; this enables
  // multiple instances of btc-rpc-explorer (eg mainnet + testnet) to share
  // a single SQLite database peacefully
  const rpcHostPort = `${config.credentials.rpc.host}:${config.credentials.rpc.port}`;
  const rpcCredKeyComponent = md5(JSON.stringify(config.credentials.rpc)).substring(0, 8);

  const sqliteCacheObj = sqliteCache.createCache(
    `${cacheKeyVersion}-${rpcCredKeyComponent}`,
    onSqliteCacheEvent
  );

  if (sqliteCacheObj) {
    miscCaches.push(sqliteCacheObj);
    blockCaches.push(sqliteCacheObj);
    txCaches.push(sqliteCacheObj);
    miningSummaryCaches.push(sqliteCacheObj);
  }
}
...
async function getNextBlockEstimate() {
  ...
  const totalFees = new Decimal(blockTemplate.coinbasevalue)
    .dividedBy(SATS_PER_BTC)
    .minus(new Decimal(subsidy));
...
const difficultyFileCache = utils.fileCache(
  config.filesystemCacheDir,
  `difficulty-by-blockheight`,
  2
);
...
function getBlockHeaderByHeight(blockHeight) {
  return tryCacheThenRpcApi(
    blockCache,
    'getBlockHeaderByHeight-' + blockHeight,
    FIFTEEN_MIN,
    function () {
...
function getMempoolTxDetails(txid, includeAncDec) {
  return tryCacheThenRpcApi(
    miscCache,
    'mempoolTxDetails-' + txid + '-' + includeAncDec,
    FIFTEEN_MIN,
    function () {
...
function buildBlockAnalysisData(blockHeight, blockHash, txids, txIndex, results, callback) {
  ...
  getRawTransactionsWithInputs([txid], -1, blockHash).then(function (txData) {
    results.push(
      summarizeBlockAnalysisData(
        blockHeight,
        txData.transactions[0],
        txData.txInputsByTransaction[txid]
      )
    );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let height = startBlock; height <= endBlock; height++) {
        ...
        summary.miners[miner].totalSubsidy = summary.miners[miner].totalSubsidy.plus(
          blockSummary.s
        );
...
function getCachedMempoolTxSummaries() {
  return new Promise(async (resolve, reject) => {
    try {
      const allTxids = await utils.timePromise(
        'coreApi_mempool_summary_getAllMempoolTxids',
        getAllMempoolTxids
      );
    ...
    try {
      ...
      for (let x = 0; x < txSummaries.length; x++) {
        ...
        if (!addedToBucket) {
          ...
          satoshiPerByteBuckets[bucketCount - 2]['totalFees'] = satoshiPerByteBuckets[
            bucketCount - 2
          ]['totalFees'].plus(new Decimal(fee).dividedBy(SATS_PER_BTC));
        ...
        let ageBucketIndex = Math.min(
          ageBucketCount - 1,
          parseInt(age / (maxAge / ageBucketCount))
        );
        let sizeBucketIndex = Math.min(
          sizeBucketCount - 1,
          parseInt(size / (maxSize / sizeBucketCount))
        );
      ...
      if (topIndex < satoshiPerByteBuckets.length) {
        ...
        for (let i = topIndex + 1; i < satoshiPerByteBuckets.length; i++) {
          ...
          satoshiPerByteBuckets[topIndex].totalFees = satoshiPerByteBuckets[
            topIndex
          ].totalFees.plus(satoshiPerByteBuckets[i].totalFees);
      ...
      for (let i = 0; i < satoshiPerByteBuckets.length; i++) {
        ...
        summary['satoshiPerByteBucketTotalFees'].push(
          summary['satoshiPerByteBuckets'][i]['totalFees']
        );
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      while (unAddedTxIndexes.length > 0 && blocks.length < 20) {
        ...
        currentBlock.avgFeeRate = currentBlock.totalFees
          .dividedBy(currentBlock.vB)
          .times(100000000)
          .toDP(1);
