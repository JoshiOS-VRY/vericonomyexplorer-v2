...
function getSmartFeeEstimate(mode = 'CONSERVATIVE', confTargetBlockCount) {
  return getRpcDataWithParams({
    ...
    parameters: [confTargetBlockCount, mode],
  });
}
...
function getNetworkHashrate(blockCount = 144) {
  return getRpcDataWithParams({ method: 'getnetworkhashps', parameters: [blockCount] }).catch(
    function (err) {
      // getnetworkhashps is not supported by all blockchain forks (e.g., Verium)
      ...
      if (isMethodNotFound) {
        debugLog(`getnetworkhashps not supported, returning null`);
        return null;
      }
...
function getBlockStats(hash) {
  if (semver.gte(global.btcNodeSemver, minRpcVersions.getblockstats)) {
    if (
      hash == coinConfig.genesisBlockHashesByNetwork[global.activeBlockchain] &&
      coinConfig.genesisBlockStatsByNetwork[global.activeBlockchain]
    ) {
...
function getRawMempool() {
  return new Promise(function (resolve, reject) {
    getRpcDataWithParams({ method: 'getrawmempool', parameters: [false] })
      .then(function (txids) {
        let promises = [];
...
function getRawMempoolEntry(txid) {
  return new Promise(function (resolve, reject) {
    getRpcDataWithParams({ method: 'getmempoolentry', parameters: [txid] })
...
function getBlockByHash(blockHash) {
  return getRpcDataWithParams({ method: 'getblock', parameters: [blockHash] })
    ...
    .catch(function (err) {
      ...
      return getRpcDataWithParams({ method: 'getblockheader', parameters: [blockHash] }).then(
        function (block) {
          block.tx = [];
          return block;
        }
...
function getRawTransaction(txid, blockhash) {
  return new Promise(function (resolve, reject) {
    if (
      coins[config.coin].genesisCoinbaseTransactionIdsByNetwork[global.activeBlockchain] &&
      txid == coins[config.coin].genesisCoinbaseTransactionIdsByNetwork[global.activeBlockchain]
    ) {
      // copy the "confirmations" field from genesis block to the genesis-coinbase tx
      getBlockchainInfo()
        .then(function (blockchainInfoResult) {
          let result =
            coins[config.coin].genesisCoinbaseTransactionsByNetwork[global.activeBlockchain];
      ...
      getRpcDataWithParams({ method: 'getrawtransaction', parameters: [txid, 1, ...extra_params] })
        .then(function (result) {
...
async function noTxIndexTransactionLookup(txid, walletOnly) {
  ...
  if (
    !walletOnly &&
    (config.addressApi == 'electrum' || config.addressApi == 'electrumx') &&
    config.electrumTxIndex
  ) {
    try {
  ...
  for (let wallet of await listWallets()) {
    try {
      return await getWalletTransaction(wallet, txid);
    } catch (_) {}
  }
  ...
  if (!walletOnly) {
    ...
    for (
      ...
      height > Math.max(tip_height - config.noTxIndexSearchDepth, 0);
      height--
    ) {
  ...
  throw new Error(
    `The requested tx ${txid} cannot be found in wallet transactions, mempool transactions, or recently confirmed transactions`
  );
}
...
async function getWalletTransaction(wallet, txid) {
  ...
  try {
    return await getRpcDataWithParams({
      method: 'gettransaction',
      parameters: [txid, true, true],
...
function getMempoolTxDetails(txid, includeAncDec = true) {
  ...
  if (includeAncDec) {
    promises.push(
      new Promise(function (resolve, reject) {
        getRpcDataWithParams({ method: 'getmempoolancestors', parameters: [txid] })
          .then(function (result) {
    ...
    promises.push(
      new Promise(function (resolve, reject) {
        getRpcDataWithParams({ method: 'getmempooldescendants', parameters: [txid] })
          .then(function (result) {
...
function getRpcData(cmd, verifyingConnection = false) {
  ...
  if (!verifyingConnection && !global.rpcConnected) {
    return Promise.reject(
      new Error('No RPC connection available. Check your connection/authentication parameters.')
    );
  }
  ...
  return new Promise(function (resolve, reject) {
    ...
    let rpcCall = async function (callback) {
      ...
      try {
        ...
        // Don't log "Method not found" errors (code -32601) - these are expected for unsupported RPC methods
        // This is common with blockchain forks like Verium that don't support all Bitcoin Core RPC methods
        let errorString = JSON.stringify(err);
        let isMethodNotFound =
          errorString.includes('-32601') || errorString.includes('Method not found');
...
function getRpcDataWithParams(request, verifyingConnection = false) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    let rpcCall = async function (callback) {
      let client =
        request.method == 'gettxoutsetinfo' ? global.rpcClientNoTimeout : global.rpcClient;
      ...
      try {
        ...
        if (!isMethodNotFound) {
          utils.logError('RpcError-002', err, {
            request: `${request.method}${request.parameters ? '(' + JSON.stringify(request.parameters) + ')' : ''}`,
          });
        }
