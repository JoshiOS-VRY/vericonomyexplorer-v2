...
const currentCoin = process.env.BTCEXP_COIN || 'VRM';
...
module.exports = {
  ...
  displayDefaults: {
    displayCurrency: process.env.BTCEXP_DISPLAY_CURRENCY || 'vrm',
  ...
  filesystemCacheDir:
    process.env.BTCEXP_FILESYSTEM_CACHE_DIR || path.join(process.cwd(), './cache'),

  sqliteEnabled:
    process.env.BTCEXP_USE_SQLITE && process.env.BTCEXP_USE_SQLITE.toLowerCase() == 'true',
  sqlitePath: process.env.BTCEXP_SQLITE_PATH || path.join(process.cwd(), './database/explorer.db'),
  ...
  rateLimiting: {
    windowMinutes:
      ...
      15,
    windowMaxRequests:
      process.env.VCEXP_RATE_LIMIT_MAX ?? process.env.BTCEXP_RATE_LIMIT_WINDOW_MAX_REQUESTS ?? 20000,
  ...
  site: {
    ...
    homepage: {
      recentBlocksCount: parseInt(
        process.env.BTCEXP_UI_HOME_PAGE_LATEST_BLOCKS_COUNT || (slowDeviceMode ? 5 : 10)
      ),
    },
    ...
    browseBlocksPageSize: parseInt(
      process.env.BTCEXP_UI_BLOCKS_PAGE_BLOCK_COUNT || (slowDeviceMode ? 10 : 25)
    ),
    ...
    prioritizedToolIdsList: [0, 10, 11, 9, 3, 4, 16, 12, 2, 5, 15, 1, 6, 7, 14, 18],
    toolSections: [
      { name: 'Basics', items: [2] },
      { name: 'Mempool', items: [4, 14, 5] },
      { name: 'Analysis', items: [8, 15, 9, 10, 11, 3] },
  ...
  siteTools: [
    ...
    /* 20 */ {
      name: 'Next Halving',
      url: './next-halving',
      desc: 'Estimated details about the next halving.',
      iconClass: 'bi-square-half',
    },
  ],
};
