...
function run() {
  ...
  if (summary.health.trusted) {
    ...
    const leaderboard = query.getLeaderboard(chainId, {
      db,
      period: 'month',
      sort: 'activity',
      limit: 5,
    ...
    const balanceHistory = query.getAddressBalanceHistory(chainId, richlist.items[0].address, {
      db,
      maxPoints: 50,
    });
    ...
    const chainActivity = query.getChainActivityHistory(chainId, {
      db,
      since: since30d,
      maxPoints: 30,
    });
