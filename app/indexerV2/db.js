...
function openDatabase(dbPath = getDatabasePath(), options = {}) {
  ...
  const busyTimeoutMs = Number(
    options.busyTimeoutMs ?? process.env.VCEXP_INDEXER_BUSY_TIMEOUT_MS ?? 30_000
  );
  db.pragma(
    `busy_timeout = ${Number.isFinite(busyTimeoutMs) && busyTimeoutMs > 0 ? busyTimeoutMs : 30_000}`
  );
...
function maybeCheckpointWal(targetDb = db, options = {}) {
  ...
  const mode = force
    ? 'TRUNCATE'
    : String(process.env.VCEXP_WAL_CHECKPOINT_MODE ?? 'PASSIVE').toUpperCase();
  const checkpointMode = ['PASSIVE', 'FULL', 'RESTART', 'TRUNCATE'].includes(mode)
    ? mode
    : 'PASSIVE';
