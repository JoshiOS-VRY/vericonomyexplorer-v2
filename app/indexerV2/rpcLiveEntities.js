...
const defaultRpcSearchDepth = Number(process.env.VCEXP_RPC_SEARCH_DEPTH) > 0
	? Number(process.env.VCEXP_RPC_SEARCH_DEPTH)
	: 500;
...
function mapRpcTransactionEntity(chainId, tx, block, txIndex, tipHeight) {
	...
	const vins = (Array.isArray(tx.vin) ? tx.vin : []).map((vin, index) => mapRpcVin(chainId, vin, index));
	const vouts = (Array.isArray(tx.vout) ? tx.vout : []).map((vout, index) => mapRpcVout(chainId, vout, index));
	...
	return {
		...
		changeOutputs: isCoinbase ? [] : vouts
...
async function lookupRpcTransaction(rpc, chainId, txid, options = {}) {
	...
	try {
		...
		return mapRpcTransactionEntity(
			chainId,
			tx,
			block,
			txIndex >= 0 ? txIndex : 0,
