...
async function main() {
	...
	for (const chain of ["vrc", "vrm"]) {
		...
		const block = tipHeight != null ? await query.getBlock(chain, String(tipHeight), { limit: 5 }) : null;
		...
		const balHist = topAddr ? await query.getAddressBalanceHistory(chain, topAddr, { maxPoints: 30 }) : null;
		const leaderboard = await query.getLeaderboard(chain, { period: "month", sort: "activity", limit: 3 });
		...
		if (tipHeight != null && chain === "vrm") {
			supply = await query.getIndexedSupplyAtHeight(chain, { height: tipHeight }).catch(() => ({ supply: null }));
		}
		...
		out[chain] = {
			...
			blockTotalsOutput: block && block.totals ? block.totals.outputValue && block.totals.outputValue.amount : null,
