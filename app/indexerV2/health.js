...
async function computeChainHealth(chainId, options = {}) {
  ...
  const unresolvedSpendCount = toNumber(
    (
      await db.get(
        `
		SELECT COUNT(*) AS count
  ...
  const expectedBlockCount =
    minIndexedHeight === null || maxIndexedHeight === null
      ? 0
...
function classify(checks, context) {
  ...
  if (
    checks.startsAtGenesis &&
    checks.noHeightGaps &&
    checks.noUnresolvedSpends &&
    checks.hasRpcTip &&
    context.blocksBehind !== null &&
    context.blocksBehind > 0
  ) {
    return {
      status: 'syncing',
      trustLevel: 'historical',
  ...
  return {
    ...
    trustLevel: 'none',
    message:
      'Address balances, richlist, and leaderboards should not be treated as authoritative yet.',
    reasons,
  };
}
...
function getTipThreshold(options) {
  const value = Number(
    options.tipThreshold || process.env.VCEXP_INDEXER_TIP_THRESHOLD || defaultOptions.tipThreshold
  );
...
function enrichWithLiveRpc(chainHealth, liveRpcHeight, options = {}) {
  ...
  const blocksBehind =
    lastIndexedHeight === null ? null : Math.max(0, liveRpcHeight - lastIndexedHeight);
