...
async function rollbackFromHeight(chainId, fromHeight, options = {}) {
  ...
  const txids = (await readStatements.findRollbackTxids.all(chainId, height)).map(
    (row) => row.txid
  );
  const affectedAddresses = (await readStatements.findAffectedAddresses.all(chainId, height)).map(
    (row) => row.address
  );
  const now = Date.now();
  const summary = {
