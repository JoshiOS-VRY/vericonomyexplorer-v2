#!/usr/bin/env bash
# Issue Let's Encrypt cert for electrumx-vrm3 and wire into /etc/electrumx-vrm3.conf
set -euo pipefail
...
install -m 755 "${SCRIPT_DIR}/renewal-hook.sh" /etc/letsencrypt/renewal-hooks/deploy/electrumx-vrm3.sh
