...
async function main() {
  ...
  const sqlitePath =
    args.sqlite || process.env.VCEXP_INDEXER_SQLITE_PATH || '/app/database/vericonomy-index.sqlite';
  ...
  const maxId = (await pool.query('SELECT COALESCE(MAX(id), 0) AS m FROM address_events')).rows[0]
    .m;
  await pool.query(
