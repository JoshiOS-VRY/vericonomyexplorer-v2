...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error: 'Indexer status unavailable',
      });
    }
...
router.get('/indexer/:chainId/richlist', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getRichlist(req.params.chainId, {
...
router.get('/indexer/:chainId/leaderboard', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getLeaderboard(req.params.chainId, {
...
router.get('/indexer/:chainId/miners', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getMinedLeaderboard(req.params.chainId, {
        period: req.query.period,
        limit: req.query.limit,
        offset: req.query.offset,
        allowUntrusted: req.query.allowUntrusted === 'true',
...
router.get('/indexer/:chainId/address/:address', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getAddress(req.params.chainId, req.params.address, {
        limit: req.query.limit,
        offset: req.query.offset,
      })
    );
...
router.get('/indexer/:chainId/activity-history', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getChainActivityHistory(req.params.chainId, {
...
router.get('/indexer/:chainId/address/:address/balance-history', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getAddressBalanceHistory(req.params.chainId, req.params.address, {
        maxPoints: req.query.maxPoints,
        since: req.query.since,
      })
    );
...
router.get('/indexer/:chainId/address/:address/utxos', function (req, res, next) {
  try {
    res.json(
      indexerQuery.getAddressUtxos(req.params.chainId, req.params.address, {
        limit: req.query.limit,
        offset: req.query.offset,
      })
    );
...
router.get(
  '/indexer/:chainId/block/:hashOrHeight',
  asyncHandler(async (req, res, next) => {
    try {
      res.json(
        await indexerSummary.getBlock(req.params.chainId, req.params.hashOrHeight, {
          limit: req.query.limit,
          offset: req.query.offset,
        })
      );
...
function handleIndexerApiError(res, err) {
  ...
  res.status(400).json({
    success: false,
    error: err.message || 'Indexer query unavailable',
  });
}
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      promises.push(
        utils.timePromise('homepage.getBlockHeaderByHeight', async () => {
      ...
      let timeAgo = moment.duration(
        moment.utc(nextHalvingData.nextHalvingDate).diff(moment.utc(new Date()))
      );
      ...
      res.json({
        ...
        nextHalvingSubsidy: coinConfig.blockRewardFunction(
          nextHalvingData.nextHalvingBlock,
          global.activeBlockchain
        ),
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (addressEncoding == 'unknown') {
        try {
          let bech32Data = bitcoinjs.address.fromBech32(addressLower);
      ...
      if (addressEncoding == 'unknown') {
        try {
          let bech32mData = bech32m.decode(addressLower);
          result.bech32m = {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      // Calculate blocks per day based on coin's target block time
      // Verium: 5 minutes = 288 blocks/day, Bitcoin: 10 minutes = 144 blocks/day
      let blocksPerDay = Math.floor((24 * 60 * 60) / coinConfig.targetBlockTimeSeconds);
      let rates = [];
...
router.get('/price/sats', function (req, res, next) {
  ...
  if (!global.exchangeRates) {
    ...
    result.error =
      "You have exchange-rate requests disabled (this is the default state; in your server configuration, you must set BTCEXP_NO_RATES to 'false', and ensure that BTCEXP_PRIVACY_MODE is also still its default value of 'false')";
  }
