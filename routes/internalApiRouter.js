...
router.get('/blocks-by-height/:blockHeights', function (req, res, next) {
  ...
  coreApi
    .getBlocksByHeight(blockHeights)
...
router.get(
  '/difficulty-by-height/:blockHeights',
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      let summary = await coreApi.buildMempoolSummary(
        statusId,
        ageBuckets,
        sizeBuckets,
        (update) => {
...
router.get(
  '/build-mining-summary/:startBlock/:endBlock',
  asyncHandler(async (req, res, next) => {
    try {
      // long timeout
      res.connection.setTimeout(600000);
