...
router.post("/search", function(req, res, next) {
	...
	// try database lookup first, then RPC validateaddress if needed
	} else if (rawCaseQuery.length >= 26 && rawCaseQuery.length <= 62 && /^[a-zA-Z0-9]+$/.test(rawCaseQuery)) {
		...
		if (config.sqliteEnabled) {
			...
			if (sqliteAddressApi && sqliteAddressApi.addressExists && sqliteAddressApi.addressExists(rawCaseQuery)) {
		...
		coreApi.getAddress(rawCaseQuery).then(function(validateResult) {
...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	...
	try {
		...
		if (!validateaddressResult.isvalid && res.locals.parseAddressErrors && res.locals.parseAddressErrors.length > 0) {
