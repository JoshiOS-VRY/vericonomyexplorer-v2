...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      promises.push(
        utils.timePromise('homepage.getBlocksByHeight', async () => {
          ...
          res.locals.blocksUntilDifficultyAdjustment =
            (res.locals.difficultyPeriod + 1) * coinConfig.difficultyAdjustmentBlockCount -
        ...
        for (let i = 0; i < chainTxStatsIntervals.length; i++) {
          promises.push(
            utils.timePromise(
              ...
              async () => {
                res.locals.chainTxStats[chainTxStatsIntervals[i][0]] =
      ...
      res.locals.difficultyAdjustmentData = utils.difficultyAdjustmentEstimates(
        eraStartBlockHeader,
        currentBlock
      );
...
// Node Details route moved to adminRouter.js - now accessible at /admin/node-details
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      let getblockchaininfo = await utils.timePromise(
      ...
      if (sort == 'desc') {
        for (
          let i = getblockchaininfo.blocks - offset;
    ...
    try {
      ...
      for (let i = 0; i < allAddresses.length; i++) {
        ...
        for (let j = 0; j < addresses.length; j++) {
          ...
          if (
            Array.isArray(addressDetailsResult.errors) &&
            ...
            if (i == 0) res.locals.receiveAddresses.push(addressDetails);
            else res.locals.changeAddresses.push(addressDetails);
          }
        }
      }
...
router.post('/search', function (req, res, next) {
  ...
  // If local parsing succeeded, redirect to address page
  if (parseAddressData.parsedAddress) {
    res.redirect('./address/' + rawCaseQuery);

    // If local parsing failed but address might still be valid (e.g., Verium addresses),
    // try database lookup first, then RPC validateaddress if needed
  } else if (
    rawCaseQuery.length >= 26 &&
    rawCaseQuery.length <= 62 &&
    /^[a-zA-Z0-9]+$/.test(rawCaseQuery)
  ) {
    // Check if it looks like an address (reasonable length, alphanumeric)

    // First, check SQLite database if enabled (fast, no RPC call)
    if (config.sqliteEnabled) {
      const sqliteAddressApi = require('./../app/api/sqliteAddressApi.js');
      if (
        sqliteAddressApi &&
        sqliteAddressApi.addressExists &&
        sqliteAddressApi.addressExists(rawCaseQuery)
      ) {
        // Address exists in database, redirect to address page (no RPC call needed!)
        res.redirect('./address/' + rawCaseQuery);
        return;
      }
    }

    // If not in database (or SQLite disabled), try RPC validateaddress
    coreApi
      .getAddress(rawCaseQuery)
      .then(function (validateResult) {
        if (validateResult && validateResult.isvalid) {
          // RPC confirms it's a valid address, redirect to address page
          res.redirect('./address/' + rawCaseQuery);
        } else {
          // Not a valid address, continue with other checks
          handleNonAddressSearch();
        }
      })
      .catch(function (err) {
        // RPC validation failed, continue with other checks
        handleNonAddressSearch();
      });

    function handleNonAddressSearch() {
      if (query.length == 64) {
        coreApi
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      {
        res.locals.metaTitle = `Verium Block #${blockHeight.toLocaleString()}`;
        res.locals.metaDesc = '';
      }
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
        ...
        res.locals.metaTitle = `Verium Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
        res.locals.metaDesc = '';
      }
      ...
      if (tx.confirmations == null) {
        ...
        promises.push(
          utils.timePromise(
            'tx.getblockheader',
            async () => {
              let rpcResult = await rpcApi.getRpcDataWithParams({
                method: 'getblockheader',
                parameters: [tx.blockhash],
              });
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      // Don't log parsing errors yet - we'll check RPC validateaddress first
      // If RPC says the address is valid, then local parsing errors are just format mismatches
      // We'll store the errors and only log them if RPC also says it's invalid
      res.locals.parseAddressErrors = parseAddressData.errors || [];
      ...
      // If RPC says the address is invalid, then log the parsing errors
      // If RPC says it's valid, then local parsing errors were just format mismatches (e.g., Verium addresses)
      if (
        !validateaddressResult.isvalid &&
        res.locals.parseAddressErrors &&
        res.locals.parseAddressErrors.length > 0
      ) {
        res.locals.parseAddressErrors.forEach((err) => {
      ...
      if (!res.locals.crawlerBot) {
        let addrScripthash = hexEnc.stringify(
                  ...
                  if (coinbaseTxs.length > 0) {
                    ...
                    blockHeightsPromises.push(
                      utils.timePromise(
                        ...
                        async () => {
                          const blocksByHashResult =
                            await coreApi.getBlocksByHash(coinbaseTxBlockHashes);
                          for (let txid in blockHashesByTxid) {
                            if (blockHashesByTxid.hasOwnProperty(txid)) {
                              blockHeightsByTxid[txid] =
                                blocksByHashResult[blockHashesByTxid[txid]].height;
                            }
                          }
                        },
                        perfResults
                      ...
                      if (txInput != null) {
                        if (txInput && txInput.scriptPubKey) {
                          if (utils.getVoutAddresses(txInput).includes(address)) {
                            ...
                            addrLossesByTx[tx.txid] = addrLossesByTx[tx.txid].plus(
                              new Decimal(txInput.value)
                            );
                          }
                        }
                      }
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      const rpcResult = await rpcApi.getRpcDataWithParams({
        method: cmd,
    ...
    try {
      ...
      if (req.query.method) {
        ...
        if (req.query.execute) {
          ...
          debugLog(
            "Executing RPC '" + req.query.method + "' with params: " + JSON.stringify(argValues)
          );
          ...
          try {
            ...
            res.locals.pageErrors.push(
              utils.logError('23roewuhfdghe', err, { method: req.query.method, params: argValues })
            );
      ...
      res.locals.pageErrors.push(
        utils.logError('23ewyf0weee', err, { method: method, params: argValues })
      );
